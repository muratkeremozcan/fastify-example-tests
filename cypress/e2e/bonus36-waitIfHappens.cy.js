// cypress/integration/bonus36.js
...
it('waits for an optional network call', () => {
  ...
  cy.visit('/calculator.html').get('#num1').type('1').get('#num2').type('2')
  ...
  cy.waitIfHappens('@calculate', 5_000).then((intercept) => {
    if (!intercept) {
      cy.log('No request')
    } else {
      cy.wrap(intercept).its('request.body').should('deep.include', {
        a: 1,
        b: 2
      })
    }
