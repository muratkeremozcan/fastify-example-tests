 ...
 */
Cypress.Commands.add('fixtureAsFile', (fixture, type, filename) => {
  cy.log(`loading **${fixture}** as ${type}`)
  return cy.fixture(fixture, null).then((buffer) => {
    const blob = Cypress.Blob.arrayBufferToBlob(buffer, type)
    return new File([blob], filename, { type })
  })
})

it('drops two fixtures', { viewportHeight: 600, viewportWidth: 1000 }, () => {
  ...
  cy.fixtureAsFile('apple.json', 'application/json', 'apple.json').then(
    (apple) => {
      cy.fixtureAsFile('bunny.png', 'image/png', 'bunny.png').then((bunny) => {
